...
describe('Financial Engine — testes obrigatórios (Seções 170-172)', () => {
  ...
  it('pedido do cliente (evolução v1.3) — Pendente a pagar/receber e Saldo projetado ficam dentro do ANO VIGENTE, nunca olhando pro ano seguinte', async () => {
    // Tenant PRÓPRIO e isolado — nunca o compartilhado deste arquivo. O
    // bug real encontrado aqui: getProjectedBalance soma o saldo real do
    // TENANT INTEIRO (todas as contas), e o tenant compartilhado já tem
    // saldo de dezenas de outros testes — supor "saldo real = 0" ali
    // estava errado. Isolando num tenant novo, o saldo real começa
    // genuinamente em zero.
    const yearPlan = await createTestPlan();
    const suffix = crypto.randomUUID().slice(0, 8);
    const { tenant } = await provisionTenantWithOwner({
      name: 'Ano Vigente Owner',
      email: `anovigente-${suffix}@example.com`,
      username: `anovigente_${suffix}`,
      planId: yearPlan.id,
    });
    const yearTenantId = tenant.id;

    const account = await createAccount(yearTenantId, {
      name: 'Conta ano vigente',
      initialBalanceCents: 0,
      initialBalanceDate: new Date('2030-01-01'),
    });

    await createIncomeOrExpense(yearTenantId, {
      type: 'EXPENSE',
    ...
    const payables2030 = await getPendingPayables(yearTenantId, 2030);
    const receivables2030 = await getPendingReceivables(yearTenantId, 2030);
    const projected2030 = await getProjectedBalance(yearTenantId, 2030);

    expect(payables2030).toBe(50_000); // só a de 2030, nunca a de 2031
    expect(receivables2030).toBe(80_000);
    expect(projected2030).toBe(0 + 80_000 - 50_000); // saldo real (genuinamente 0, tenant isolado) + a receber - a pagar, só 2030

    const payables2031 = await getPendingPayables(yearTenantId, 2031);
    expect(payables2031).toBe(30_000); // a de 2031 só conta pro ano dela

    await prisma.financialTransaction.deleteMany({ where: { tenantId: yearTenantId } });
    await prisma.financialAccount.deleteMany({ where: { tenantId: yearTenantId } });
    await cleanupTenant(yearTenantId);
    await deleteTestPlan(yearPlan.id);
  });
});
