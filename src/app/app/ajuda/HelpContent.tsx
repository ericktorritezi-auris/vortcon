...
const SECTIONS: HelpSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      {
        question: 'O que são os botões +/- e o "Histórico do valor" na edição de uma transação?',
        answer:
          'É útil pra quem cria um lançamento de planejamento (ex.: cria "Imprevistos do mês" no dia 1º com R$ 0,01) e vai somando/subtraindo valores ao longo do mês, conforme os gastos acontecem. Na edição, ao lado do campo Valor, tem um botão "−" e um "+". Clique em um deles, digite o valor do ajuste e confirme no ✓ — o valor total da transação já muda na hora, e uma linha nova aparece em "Histórico do valor" com a data (sempre automática, nunca digitada) e o valor daquele ajuste. Editar o campo Valor direto, sem usar +/-, continua funcionando exatamente como sempre — só não gera linha nenhuma no histórico. Lançou um ajuste errado? Na edição, cada linha do histórico tem um ✕ — clique nele pra excluir e o valor total é recalculado na hora, desconsiderando aquele ajuste. O histórico é só um "diário" daquela transação (dia + valor, sem descrição) — nunca afeta nenhum cálculo do Financial Engine, relatório ou outra transação.',
      },
      {
