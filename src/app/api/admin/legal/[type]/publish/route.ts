...
import { publishDraft } from '@/modules/legal/legal-document.service';

const publishSchema = z.object({ requiresReacceptance: z.boolean() });
...
export async function POST(
  ...
  try {
    const published = await publishDraft(documentType, parsed.data.requiresReacceptance);
    return NextResponse.json({ status: 'ok', version: published.version });
  } catch {
    return NextResponse.json(
      { error: 'NO_DRAFT', message: 'Não há rascunho pendente para publicar.' },
      { status: 409 },
    );
  }
}
